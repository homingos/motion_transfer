# FLAM Motion Transfer - Production Environment Usage Guide

## Production URL

```
https://flam--motion-transfer.modal.run
```
...
**Example:**
```bash
curl -X POST https://flam--motion-transfer.modal.run/generate \
...
**Example:**
```bash
curl -X POST https://flam--motion-transfer.modal.run/idle-motion \
...
**Example:**
```bash
curl https://flam--motion-transfer.modal.run/jobs/2d191f4269e7
```
...
**Example:**
```bash
curl -O https://flam--motion-transfer.modal.run/jobs/2d191f4269e7/result
```
...
**Example:**
```bash
curl -O https://flam--motion-transfer.modal.run/reference/man.mp4
```
...
```bash
while true; do
  STATUS=$(curl -s https://flam--motion-transfer.modal.run/jobs/$REQUEST_ID | jq -r '.status')
  
  if [ "$STATUS" = "done" ]; then
    echo "✅ Completed!"
    break
  elif [ "$STATUS" = "failed" ]; then
    ERROR=$(curl -s https://flam--motion-transfer.modal.run/jobs/$REQUEST_ID | jq -r '.error')
    echo "❌ Failed: $ERROR"
    break
  fi
...
### Step 1: Submit Job
```bash
REQUEST_ID=$(curl -s -X POST https://flam--motion-transfer.modal.run/idle-motion \
...
```bash
curl -O https://flam--motion-transfer.modal.run/jobs/$REQUEST_ID/result
```

---

## Default Settings (Production Environment)
...
## Testing Quick Links
 - **Max concurrent jobs**: 1 per GPU container (scales to 20 containers on production)
 - **UI and curl outputs are equivalent**: Both use the same pipeline with the same defaults. UI uses `/generate` (file upload), curl typically uses `/idle-motion` (GCS URL) — both produce ~4 second videos.

### Test with Different Images
```bash
# Test 1
curl -s -X POST https://flam--motion-transfer.modal.run/idle-motion \
  -F "image_url=https://storage.googleapis.com/bucket-fi-production-apps-0672ab2d/original/images/owaibstq8bfds8zflbok20qm.png" \
  -F "gender=male"
